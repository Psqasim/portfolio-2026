          ...
          {skillCategories.map((category, idx) => {
            ...
            return (
              <div key={category.slug} data-testid="skill-category">
                ...
                <InfiniteMarquee direction={direction} speedSeconds={28}>
                  {category.skills.map((skill) => (
                    <div
                      key={skill.name}
                      data-testid="skill-item"
                      className="flex shrink-0 items-center gap-3 rounded-md border border-[var(--color-border)] bg-[var(--color-card)] px-3 py-2 text-sm text-[var(--color-text)] transition hover:border-[var(--color-accent-cyan)] hover:shadow-[0_0_16px_-4px_var(--color-accent-cyan)]"
                    >
                      <span
                        aria-hidden
